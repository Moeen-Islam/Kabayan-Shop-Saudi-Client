  ...
  return (
    <div className="min-h-screen bg-neutral-50 flex flex-col justify-between selection:bg-amber-400 selection:text-black overflow-x-hidden w-full max-w-full">
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 z-40 flex items-center justify-center bg-emerald-500 hover:bg-emerald-600 text-white p-2.5 rounded-full shadow-2xl hover:scale-110 transition-all duration-300 group cursor-pointer"
      ...
      >
        ...
        <svg className="w-5.5 h-5.5 text-white fill-current group-hover:rotate-12 transition-transform duration-300" viewBox="0 0 24 24">
      ...
      {/* E. FLOATING MESSENGER CHAT BUTTON */}
      {(settings?.messengerPageId || "kabayanshopSaudi1") && (
        <>
          ...
          >
            ...
            <svg className="w-5.5 h-5.5 text-white fill-white group-hover:scale-110 transition-transform duration-300" viewBox="0 0 24 24" fill="currentColor">
      ...
      <button
        ...
        id="cart-float-btn"
      >
        <ShoppingBag className="w-5.5 h-5.5 group-hover:scale-115 transition-transform duration-300" />
        {items.length > 0 && (
          <span className="absolute -top-1.5 -right-1.5 bg-black text-white text-[9px] font-black w-4.5 h-4.5 rounded-full flex items-center justify-center border border-white">
            {items.reduce((acc, item) => acc + item.quantity, 0)}
          </span>
        )}
        <span className="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-500 ease-out whitespace-nowrap text-xs font-extrabold uppercase tracking-wider pl-0 group-hover:pl-2">
          View Cart
        </span>
      </button>
